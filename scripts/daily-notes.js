...
function generateIssueMarkdown(issue) {
  ...
  fs.writeFile(`./docs/${repo}/issue-${issue.number}.md`, content, "utf8").then(() => console.log(`Issue ${issue.number} generated successfully!`));
}
...
共计 **${issueYearGroups.reduce((total, [, issues]) => total + issues.length, 0)}** 篇（上次更新: ${formatTime(issueYearGroups[0][1][0].created_at)}）
...
${issues
  ...
  .map((issue, index) => `${index + 1}. ${formatTime(issue.created_at)} —— [${issue.title}](${`/daily-notes/issue-${issue.number}`})`)
  .join("\n\n")}`
  )
  .join("\n\n")}
`;

  fs.writeFile("./docs/daily-notes/index.md", content, "utf8").then(() => console.log("Index file generated successfully!"));
